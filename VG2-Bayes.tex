...
JOE, ON A MINOR POINT, I AM NOT A FAN OF USING BOTH R AND DELTA.  I THINK IT IS LIKE USING METRIC AND ENGLISH UNITS IN THE SAME PAPER.  I WOULD PROBABLY DO EVERYTHING IN TERMS OF DELTA AND JUST NOTE IT THE FIRST TIME YOU CITE A RESULT IN R. ALTERNATIVELY, YOU SHOULD COULD WARN READERS THAT YOU WILL BE SWITCHING AS NEEDED AND JUST PROVIDE THE RELATIONS BETWEEN THE TWO EFFECT SIZE MEASURES.  SOMETHING IS NEEDED THOUGH.

JOE, YOU NEED FAR MORE INFORMATIVE FIGURE CAPTIONS.
...
The need for methods to state evidence for the null manifest in two distinct ways in the study of aggression from violent video games.  First, if the no-effect null is going to be treated as {\em a priori} plausible and on a level playing field with the alternative,e then there needs to be a way of favoring it over the alternative.  Second, if games are going to be matched on confounding variables, there needs to be a way of favoring the no-difference null over an alternative.  So crucial is matching in fact that it is considered part a ``best practice" by \citet{Anderson:etal:2010}.    IF YOU LIKE IT YOU CAN COMBINE WITH THE FOLLOWING PARAGRAPH INTO ONE PARAGRAPH.
...
\end{quote}
If the obtained Bayes factor is not sufficiently large, more data can be collected. While such optional or conditional stopping is a serious and dangerous form of research flexibility in NHST \citep{Simmons:etal:2011}, it is not a problem for Bayes factor \citep{Dienes:2011,Rouder:2014}. %Also could \citep{} 
Thus, data could potentially be freely collected until the obtained Bayes factor is satisfyingly convincing.

% Might comment out rest of paragraph too:  YEAH, NOT NEEDED.
We will point out that Bayes factors are typically larger when hypotheses are more different from each other. Thus, it can be easy to provide evidence for the null over a hypothesized large effect, but it will be difficult to provide evidence for a null over a hypothesized small effect, as the predictions of the null and alternative hypotheses are highly similar. Some see this as a flaw of Bayes factor, as the obtained Bayes factor depends on the alternative hypothesis. We see this instead as part of the intended behavior. Researchers squint with skepticism when an effect is reported as statistically significant with a colossal effect size in a very small sample. This is because they do not believe that the effect is actually that large. Bayesian modeling makes this implicit subjectivity into an explicit modeling statement. 
...
With these Bayes factors, researchers can assess the relative strength for all three hypotheses. If $B_{02} < 1$, the results replicate and support the meta-analytic findings. If $B_{02} > 1$, the results provide evidence for the null hypothesis, indicating that the null is more likely than the meta-analytic alternative, given the observed data.  If $B_{02} \approx 1.0$, then the data do not discriminate between the two hypotheses. Comparisons between $H_{A1}$ and $H_0$ or $H_{A1}$ and $H_{A2}$ could indicate evidence for an effect of a magnitude or direction not predicted by $H_{A2}$. This model comparison between the null and meta-analytic alternative is applicable in many research contexts in which researchers explore the mediators, boundaries, or potential confounds associated with a psychological phenomenon.  

I THINK READERS ARE GOING TO HAVE A HARD TIME UNDERSTANDING THE NEED, CONSEQUENCES, AND ADVANTAGES OF MULTIPLE ALTERNATIVES.  IT IS NOT WELL LAID OUT.  NEEDS MORE MOTIVATION.
...
\caption{Bayes factors by effect size.}
\label{BFNfig}
\end{figure}
%{\em Note:} Solid lines represent a study of $n=40$ in two cells, dotted lines represent a study of $n=200$ in two cells.
% image dimensions far exceed table dimensions
...
\begin{figure}
\includegraphics[width=\textwidth, keepaspectratio]{BFfigure.pdf}
\caption{Bayesian model comparison.  VAGUE, NEED A BETTER ONE.  RULE OF THUMB IS THAT AN EDUCATED READER CAN JUST SKIM THE FIGS AND TABLES AND UNDERSTAND THE PAPER TO SOME DEGREE.}
