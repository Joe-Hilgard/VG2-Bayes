...
% Jeff suggests new section header here, saying "We change ideas pretty hard and the reader may struggle to keep up"
...
% Reviewer 1 complains that we make it sound like NHST is fine when you get significance, not fine otherwise.
...
% Jeff suggests that he doesn't like this next sentence.
Finally, results are summarized and used to inform practical suggestions offered for stronger, more informative research. 
% This is too pat.
The present manuscript is not intended as a systematic review, but the sample of studies analyzed is nonetheless representative.
...
However, we note that pilot tests rarely include more than 40 subjects. In a 40-subject study, even an observed effect of exactly $r = .00$ would yield at most $B_{01} = 1.87$ in a between-subjects comparison or $B_{01} = 3.32$ in a within-subjects comparison. Thus, pilot tests are generally too small to provide much support for the null, and so the above examples are likely to be representative of a larger problem.
...
\citet{Bartholow:etal:2005}, and \citet{Hasan:etal:2012}. In other cases, a pilot test was conducted but statistics not provided \citep[e.g.,][]{Engelhardt:etal:2011}. 

% Re-write me!
While this manuscript does not constitute a systematic review, we nonetheless expect that few studies provide considerable evidence of matching. To support this expectation, we provide in Figure~\ref{BFMinMax} plots of the maximum and minimum possible evidence for the null hypothesis for independent-groups and repeated-measures $t$-tests of pilot studies with $p > .05$. Even in the best-case scenario that $p = 1.0$, one needs at least 33 subjects per independent group or 17 subjects in a repeated-samples group to achieve 3:1 evidence for the null.  Pilot studies of that size are rare in this literature.
...
Thus, much as in the insufficient pretesting examples above, one cannot be sure that this approach completely eliminates the effect of confounds on the dependent variable.
...
%Is it necessary to riposte Reviewer 1 in-text by explaining that these were the two results reported?
Thus, re-examination of the reported effect of violent game content in \citet{Elson:etal:2013} indicates an agnostic result (effects on mean noise duration) and a moderately informative replication (effects on mean noise intensity). The non-significant result has been misinterpreted as support for the null when instead support has been found for the alternative.  
...
%"theoretically meaningful alternative" is such a lame mouthful
...
% Ferugson will again have a fit about this paragraph. What to do?
