...
text(1,.25,expression(paste("Model ",M[a])),col="darkgreen",adj=0)
mtext(side=3,adj=.05,cex=1.7,'A.',line=-1.1)
...
text(.9,.07,expression(paste("Model ",M[a])),col="darkgreen",adj=0)
...
mtext(side=3,adj=.05,cex=1.7,'B.',line=-1.1)
...
plot(x=effect, xlab = expression(paste("Observed Effect Size, ",(bar(y)-bar(x))/s)),
     y=BFList, log="y", ylim=c(.00001, 10), ylab="Bayes factor (null/alternative)",
     typ='l',
     yaxt='n'
     )
